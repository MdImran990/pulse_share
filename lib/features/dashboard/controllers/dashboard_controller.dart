...
class DashboardController extends GetxController {
  ...
  // ================= SIM =================
  final RxString simCarrier = "--".obs;
  final RxString simState = "--".obs;
  final RxString simSignal = "--".obs;
  ...
  Future<void> checkPermissionsAndLoad() async {
  ...
  Future<void> loadDeviceInfo() async {
    try {
      final DeviceInfoModel device = await _repository.getDeviceInfo();
      deviceModel.value = device.model;
      androidVersion.value = "Android ${device.androidVersion}";
      deviceName.value = device.brand;
    } catch (e) {
      Get.snackbar("Device Error", e.toString());
    }
  }
  ...
  Future<void> loadWifiInfo() async {
    try {
      final NetworkInfoModel wifi = await _repository.getWifiInfo();
      wifiName.value = wifi.wifiName;
      ipAddress.value = wifi.ipAddress;
      wifiSignal.value = wifi.wifiSignal;
    } catch (e) {
      Get.snackbar("WiFi Error", e.toString());
    }
  }
  ...
  Future<void> loadSensorInfo() async {
    try {
      final SensorInfoModel sensor = await _repository.getSensorInfo();
      stepCount.value = sensor.stepCount;
      activity.value = sensor.activity;
    } catch (e) {
      Get.snackbar("Sensor Error", e.toString());
    }
  }
  ...
  Future<void> loadSimInfo() async {
    try {
      final sim = await _repository.getSimInfo();
      simCarrier.value = sim["carrierName"] ?? "Unknown";
      simState.value = sim["simState"] ?? "Unknown";
      simSignal.value = sim["signalStrength"] ?? "--";
    } catch (e) {
      Get.snackbar("SIM Error", e.toString());
    }
  }
